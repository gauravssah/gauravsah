...
export function HeroSkillsShowcase({ name, role }: HeroSkillsShowcaseProps) {
  return (
    <div className="relative h-[23rem] w-full overflow-hidden rounded-[2rem] border border-white/10 bg-[#07122b]/95 p-6 shadow-glow lg:h-[37rem]">
      ...
      <div className="relative z-10 flex h-full items-start justify-center pt-8 [perspective:1200px] lg:pt-10">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.65 }}
          whileHover={{ rotateY: 7, rotateX: -5, y: -5 }}
          className="w-[min(88%,340px)] rounded-3xl border border-white/15 bg-white/5 p-5 backdrop-blur-md"
        ...
        >
          ...
          <p className="mt-1 text-sm text-cyan/80">{role}</p>
      ...
      {floatingSkills.map((item) => (
        <motion.div
          key={item.label}
          className="absolute z-20 rounded-2xl border border-white/10 bg-white/10 px-3 py-2 text-xs text-white/85 backdrop-blur-md"
